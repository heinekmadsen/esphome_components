...
void WavinAhc9000Climate::setup() {
  ...
  wavinAhc9000_->add_target_temp_callback([this](float state) {
    target_temperature = state;
    publish_state();
  });
  wavinAhc9000_->add_fan_speed_callback([this](int state) {
...
void WavinAhc9000Climate::control(const climate::ClimateCall &call) {
  if (call.get_target_temperature().has_value())
  {
	  ...
	  wavinAhc9000_->writeTargetTemperature(target);
  }
  ...
  if (call.get_fan_mode().has_value())
  {
	  ...
	  wavinAhc9000_->writeFanMode(mode);
  }
  this->publish_state();
}
